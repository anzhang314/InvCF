...
from model import CausE, IPS, LGN, MACR, INFONCE_batch, DEBIAS, INFONCE, DEBIAS_batch, IPSMF, DEBIAS_BPR, BC_LOSS, BC_LOSS_batch, SimpleX, SimpleX_batch, sDRO, sDRO_batch, DEBIAS_plot, LGN_plot, CDAN, CDAN_MF
...
if __name__ == '__main__':
    ...
    if args.modeltype == "DEBIAS" or args.modeltype == 'DEBIAS_batch' or args.modeltype == 'DEBIAS_BPR':
        saveID += "_pop=" + str(args.lambda1) +  "_disc=" + str(args.lambda2) + "_sub=" + str(args.lambda3) + "tau=" + str(args.tau)
    if args.modeltype == "DEBIAS_plot" :
    ...
    if not args.pop_test:
        ...
        eval_test_ood = ProxyEvaluator(data,data.train_user_list,data.test_ood_user_list,top_k=[20],\
                            dump_dict=merge_user_list([data.train_user_list,data.valid_user_list,]),pop_mask=pop_mask)
        eval_valid = ProxyEvaluator(data,data.train_user_list,data.valid_user_list,top_k=[20])
    ...
    if args.modeltype == 'DEBIAS_plot':
        model = DEBIAS_plot(args, data)
    if args.modeltype == 'LGN_plot':
        model = LGN_plot(args, data)
...
#    b=args.sample_beta
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_i, batch in pbar:        
            ...
            elif args.modeltype == 'DEBIAS' or args.modeltype == 'DEBIAS_BPR':
            ...
            elif args.modeltype == 'DEBIAS_plot':
                mf_loss, reg_loss  = model(users, pos_items, neg_items, users_pop, pos_items_pop, neg_items_pop)
                loss = mf_loss + reg_loss
            
            elif args.modeltype == 'LGN_plot':
                mf_loss, reg_loss  = model(users, pos_items, neg_items, users_pop, pos_items_pop, neg_items_pop)
                loss = mf_loss + reg_loss
        ...
        elif args.modeltype == "DEBIAS" or args.modeltype == 'DEBIAS_batch' or args.modeltype == 'DEBIAS_BPR':          
