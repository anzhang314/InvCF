...
class Data:
    ...
    def __init__(self, args):
        ...
        # for ablation study
        self.head, self.mid, self.tail, self.no_head, self.no_mid, self.no_tail = [], [], [], [], [], []
        self.len_head, self.len_mid, self.len_tail = 0, 0, 0
        self.head_user, self.mid_user, self.tail_user, self.no_head_user, self.no_mid_user, self.no_tail_user = [], [], [], [], [], []
        self.len_head_user, self.len_mid_user, self.len_tail_user = 0, 0, 0
        self.idx, self.idx_user = [], []

        self.mid_head, self.mid_tail, self.head_tail = 0, 0, 0 

        self.mid_head_u, self.mid_tail_u, self.head_tail_u = 0, 0, 0
    ...
    def load_data(self):
        ...
        self.group_identity = list(user_group_dict.values())
         ...
         # group data  (for ablation study)
        p_item = np.array([len(self.train_item_list[u]) if u in self.train_item_list else 0 for u in range(self.n_items)])
        p_user = np.array([len(self.train_user_list[u]) if u in self.train_user_list else 0 for u in range(self.n_users)])
        ...
        for grp in range(n_groups):
            split=int((self.n_items-1)*(grp+1)/n_groups)
            print(split)
            grp_view.append(pop_sorted[split])
        idx=np.searchsorted(grp_view,p_item)
        self.idx = idx

        self.head = np.where(idx == 2)[0]
        self.len_head = self.head.shape[0]
        self.mid = np.where(idx == 1)[0]
        self.len_mid = self.mid.shape[0]
        self.tail = np.where(idx == 0)[0]
        self.len_tail = self.tail.shape[0]
        self.no_head = np.concatenate((self.mid, self.tail))
        self.no_mid = np.concatenate((self.head, self.tail))
        self.no_tail = np.concatenate((self.head, self.mid))
        ...
        grp_view=[]
        for grp in range(n_groups):
            split=int((self.n_users-1)*(grp+1)/n_groups)
            print(split)
            grp_view.append(pop_sorted[split])
        idx=np.searchsorted(grp_view,p_user)
        self.idx_user = idx

        self.head_user = np.where(idx == 2)[0]
        self.len_head_user = self.head_user.shape[0]
        self.mid_user = np.where(idx == 1)[0]
        self.len_mid_user = self.mid_user.shape[0]
        self.tail_user = np.where(idx == 0)[0]
        self.len_tail_user = self.tail_user.shape[0]
        self.no_head_user = np.concatenate((self.mid_user, self.tail_user))
        self.no_mid_user = np.concatenate((self.head_user, self.tail_user))
        self.no_tail_user = np.concatenate((self.head_user, self.mid_user))

        self.mid_head = self.len_mid/(self.len_mid + self.len_head)
        self.mid_tail = self.len_mid/(self.len_mid + self.len_tail)
        self.head_tail = self.len_head/(self.len_head + self.len_tail)
    
        self.mid_head_u = self.len_mid_user/(self.len_mid_user + self.len_head_user)
        self.mid_tail_u = self.len_mid_user/(self.len_mid_user + self.len_tail_user)
        self.head_tail_u = self.len_head_user/(self.len_head_user + self.len_tail_user)
